...
char constexpr      lf = 0x0A;       // Unix line ending

void                targets(ostream & ofs,string const & prjName,ConsSrcDir const & grp)
{
...
void                group(ostream & ofs,string const & prjName,const ConsSrcDir & grp)
{
    string  odir = "$(ODIR" + prjName + ")" + replaceAll(grp.dir,'/','_'),
...
string              targetPath(ConsProj const & prj)
{
    if (prj.isDynamicLib())
...
bool                constIncludeFileNative(ConsSolution const & sln,string const & fname)
{
...
//bool                constIncludeFileBox(ConsSolution const & sln,string const & fname)
//{
//    ostringstream       ofs;
//    ofs
//        // .PHONY ensures 'make' won't check for a file named 'all' so will always update:
    ...
    string const &      fnameBuild)         // includes this makefile
{
    ...
    if (os == BuildOS::ios) {
        ...
        else if (arch == Arch::arm7)
            cflags.push_back("-arch armv7");
        else if (arch == Arch::arm8)
        ...
        if (arch == Arch::x86)
            ...
            cflags.push_back("-march=armv7-a");
        else if (arch == Arch::arm8)
            cflags.push_back("-march=armv8-a");
        //else if (arch == Arch::arm8_3)
        //    cflags.push_back("-march=armv8.3-a");
        else
    ...
    if (compiler == Compiler::clang) {
        ...
        // gcc limit of 1024 too small due to large ACS thread sigs:
...
//bool                consMakefileIos(ConsSolution const & sln)
//{
...
//bool                consMakefileAndroid(ConsSolution const &)
//{
...
{
    FGASSERT(sln.type == ConsType::nix);    // Windows is built with visual studio files
    bool            changed = false;
    string          fnameAll = "make_all.mk";
    changed = constIncludeFileNative(sln,fnameAll) || changed;
    for (Debrel debrel : debrels) {
        for (auto compiler : getBuildCompilers(BuildOS::linux)) {
            changed = consMakefileOsArch(BuildOS::linux,compiler,Arch::x64,debrel,fnameAll) || changed;
            changed = consMakefileOsArch(BuildOS::linux,compiler,Arch::arm8,debrel,fnameAll) || changed;
        }
