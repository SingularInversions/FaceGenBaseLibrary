    ...
    Offsets             offsets)
{
    ...
    for (uint tt=0; tt<mesh.texImages.size(); ++tt) {
        string  idxString = fgToString(offsets.mat+tt),
                imgExt;
        if (fgUsesAlpha(mesh.texImages[tt]))
            imgExt = ".png";
        else
            imgExt += ".jpg";
        // Some OBJ parsers (Meshlab) can't handle spaces in filename:
        FgString        imgName = fpath.base.replace(' ','_')+idxString+imgExt;
        fgSaveImgAnyFormat(fpath.dirOnly()+imgName,mesh.texImages[tt]);
        if (ofsMtl) {
    ...
    const std::vector<Fg3dMesh> &   meshes)
{
    ...
    ofs <<
        ...
        "# Generated by FaceGen tools, for more information visit www.FaceGen.com.\n";
    ...
    if (texImage) {
        ...
        ofsMtl.open(fpath.dirOnly() + mtlBaseExt);
    ...
    for (size_t ii=0; ii<meshes.size(); ++ii) {
        ...
        offsets = writeMesh(ofs,ofsMtl,meshes[ii],fpath,offsets);
    }
}
