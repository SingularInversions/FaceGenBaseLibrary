...
testVariantB()
{
    ...
    FGASSERT(ii == 7);
        ...
        // Test that variants do deep copies
    {
        FgVariant v1(FgVect2D(1234,5678));
        FgVariant v2(v1); // should deep copy
        
        FgVect2D& pv1(v1.getRef<FgVect2D>());
        FgVect2D& pv2(v2.getRef<FgVect2D>());

        FGASSERT(pv1[0] == pv2[0]);
        FGASSERT(pv1[1] == pv2[1]);
        pv1[0] = 99;
            // NOW THE VALUES SHOULD BE DIFFERENT!
        FGASSERT(pv1[0] != pv2[0]);
        FGASSERT(pv1[1] == pv2[1]);
    }
